...
filename =  "PP_Her_PPnew_NTU_EvpaDes_Temp_m6_const"
...
# ---------------------------------------------------------------------------
M6_FIXED   = 1.0    # Startwert für den ersten Betriebspunkt
# ---------------------------------------------------------------------------
...
def optimize_m6_at(scan_T_C, m6_seed, x0_seed_K, build_fn, solve_fn, label):
    res = solve_fn(build_fn(scan_T_C, M6_FIXED), x0=x0_seed_K)

    if is_converged(res):
        return M6_FIXED, res, result_to_x0(res)

    return M6_FIXED, None, x0_seed_K
...
def run_sweep(build_fn, solve_fn, label: str, model: str = "UA") -> dict[float, dict]:
    ...
    m6_c, x0_c = solve_and_store(float(temperatures[idx_start]), M6_FIXED, x0_K)
