...
filename =  "PP_Her_UA_EvpaDes_Q_abs_const"
...
# ---------------------------------------------------------------------------
M6_INITIAL   = 2.8    # Startwert für den ersten Betriebspunkt
M6_ABS_MIN   = 0.05    # absolutes Untergrenze für m6 [kg/s]
M6_ABS_MAX   = 6.0     # absolutes Obergrenze für m6 [kg/s]
...
def optimize_m6_at(scan_T_C, m6_seed, x0_seed_K, build_fn, solve_fn, label):
    target_Q = Q_ABS_TARGET   # vorgegebener Sollwert
    ...
    def objective(m6):
        ...
        if best is None or err < abs(best.heat_flows_kW.get("Q_abs", 0.0) - target_Q):
            best_store.update(result=res, x0=result_to_x0(res), m6=m6)
        return err

    minimize_scalar(objective, bounds=(lo, hi), method="bounded",
