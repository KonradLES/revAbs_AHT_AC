...
- keine Flash-Bildung an der Lösungsdrossel
...
def _evaluate_model_common(z: np.ndarray, inputs: AWTInputs, *, strict: bool) -> ModelEvaluation:
    ...
    residuals_raw_array = np.array(
        [
            ...
            Q_shex - inputs.UA_shex * lmtd_shex,
...
def trace_model(z: np.ndarray, inputs: AWTInputs) -> ModelTrace:
    ...
    try:
        ...
        lmtd_des = counterflow_lmtd(
            hot_in=T13_in, hot_out=T14, cold_in=T1, cold_out=T6
        )
        ...
        lmtd_abs = counterflow_lmtd(hot_in=T20, hot_out=T3, cold_in=inputs.T_11, cold_out=T12)
